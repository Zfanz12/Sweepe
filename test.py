...
def show_navbar(back_fn=None, show_done_btn=False):
...
def show_landing():
    ...
    if _resume_session:
        ...
        resume_card = ctk.CTkFrame(app, fg_color=BG_CARD, corner_radius=14,
        ...
        ctk.CTkLabel(text_block, text=f'Resume  "{folder_name}"',
                     font=("Georgia", 12, "bold"), text_color=AMBER).pack(anchor="w")
        ...
        ctk.CTkButton(btn_block, text="Lanjut →", command=do_resume,
                      ...
                      text_color="#0c0c0e", font=("Georgia", 11, "bold"),
...
def _resume_now():
    ...
    for w in [label, sub, divider_line, how_label, steps_frame, btn, mulai_label]:
        w.place_forget()
    maximize_window()
    show_navbar(back_fn=_ask_back_from_viewer, show_done_btn=True)
...
def _make_popup(w, h, title, body, buttons, extra_widget_fn=None):
    ...
    ctk.CTkLabel(popup, text=title,
                 font=("Georgia", 16, "bold"), text_color=TEXT).place(relx=0.5, rely=0.22, anchor="center")
...
def _ask_back_from_viewer():
    ...
    ctk.CTkLabel(popup, text="Keluar dari sesi ini?",
                 font=("Georgia", 16, "bold"), text_color=TEXT).place(relx=0.5, rely=0.20, anchor="center")
    ...
    ctk.CTkButton(row, text="Lanjut beberes", command=popup.destroy,
                  ...
                  text_color=TEXT, font=("Georgia", 11, "bold"),
    ...
    ctk.CTkButton(row, text="Keluar tanpa simpan", command=do_discard_exit,
                  ...
                  text_color=ROSE, font=("Georgia", 11, "bold"),
...
def show_sort_screen(folder):
    ...
    for i, (title, desc, buttons) in enumerate(sort_options):
        ...
        ctk.CTkLabel(card, text=title,
                     font=("Georgia", 14, "bold"), text_color=TEXT).pack(pady=(12, 2))
...
def show_summary():
    ...
    tag(ctk.CTkLabel(app, text="Review & Konfirmasi",
                     font=("Georgia", 22, "bold"), text_color=TEXT)
    ...
    for col, (lbl, val, color) in enumerate(pill_data):
        pill = ctk.CTkFrame(pills, fg_color=BG_CARD, corner_radius=10,
        ...
        ctk.CTkLabel(pill, text=str(val),
                     font=("Georgia", 26, "bold"), text_color=color).pack(pady=(6,0))
    ...
    ctk.CTkLabel(hdr, text="Files yang akan dihapus",
                 font=("Georgia", 11, "bold"), text_color=ROSE).pack(side="left")
    ...
    ctk.CTkButton(btn_row, text="← Cek lagi", command=go_back_to_viewer,
                  fg_color=BTN_SEC, hover_color=BTN_SEC_HOV,
                  text_color=TEXT, font=("Georgia", 12, "bold"),
...
def confirm_delete(list_card=None, btn_row=None):
    ...
    ctk.CTkLabel(popup, text="Berhasil dihapus",
                 font=("Georgia", 17, "bold"), text_color=TEAL).place(relx=0.5, rely=0.26, anchor="center")
...
nav_title = ctk.CTkLabel(navbar, text="SWEEPERY",
                          font=("Georgia", 13, "bold"), text_color=AMBER)
...
ctk.CTkLabel(left_panel, text="←",
             font=("Georgia", 40, "bold"), text_color=ROSE).place(relx=0.5, rely=0.36, anchor="center")
ctk.CTkLabel(left_panel, text="HAPUS",
             font=("Consolas", 11, "bold"), text_color=ROSE).place(relx=0.5, rely=0.46, anchor="center")
ctk.CTkLabel(left_panel, text="[ ← ]",
...
ctk.CTkLabel(right_panel, text="→",
             font=("Georgia", 40, "bold"), text_color=TEAL).place(relx=0.5, rely=0.36, anchor="center")
ctk.CTkLabel(right_panel, text="SIMPAN",
             font=("Consolas", 11, "bold"), text_color=TEAL).place(relx=0.5, rely=0.46, anchor="center")
ctk.CTkLabel(right_panel, text="[ → ]",
...
for i, (num, title, desc) in enumerate(steps):
    ...
    ctk.CTkLabel(card, text=title,
                 font=("Georgia", 13, "bold"), text_color=TEXT).pack()
    ...
    if i < len(steps) - 1:
        sep = ctk.CTkLabel(steps_frame, text="·",
                           font=("Georgia", 18), text_color=BORDER)
...
btn = ctk.CTkButton(app, text="Pilih Folder  →", command=pick_folder,
                    fg_color=AMBER, hover_color=AMBER_HOV,
                    text_color="#0c0c0e", font=("Georgia", 15, "bold"),
...
counter_label  = ctk.CTkLabel(app, text="",
                               font=("Georgia", 14, "bold"), text_color=AMBER)

undo_btn = ctk.CTkButton(app, text="Undo  ↓", command=undo,
                          fg_color="transparent", hover_color=BTN_SEC_HOV,
                          text_color=TEXT_DIM, font=("Consolas", 11, "bold"),
                          border_width=1, border_color=BORDER_LT,
                          height=52, width=800, corner_radius=0)
...
app.bind("<Left>",  swipe_delete)
app.bind("<Right>", swipe_keep)
app.bind("<Down>",  undo)
app.bind("<Up>",    skip)
